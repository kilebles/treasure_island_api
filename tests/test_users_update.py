...
async def test_update_user_data_valid(client: AsyncClient):
    ...
    data = response.json()
    assert data["user"]["fullName"] == payload["full_name"]
    assert data["user"]["phoneNumber"] == payload["phone_number"]
    assert data["user"]["inn"] == payload["inn"]

    await user.fetch_related("profile")
    assert user.profile.full_name == payload["full_name"]
    assert user.profile.phone_number == payload["phone_number"]
